...
def plot_all_data(folder, game, name, num_steps, bin_size=(10, 100, 100, 1), smooth=1, time=None, save_filename='results.png', ipynb=False):
    ...
    if time is not None:
        title = 'Avg. Last 10 Rewards: ' + \
            str(np.round(np.mean(ty[-10]))) + ' || ' + \
            game + ' || Elapsed Time: ' + str(time)
    else:
        title = 'Avg. Last 10 Rewards: ' + \
    ...
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(20, 15), subplot_kw=dict(
    ...
    if tx is not None or ty is not None:
        ...
        for percent, index in zip(ty, range(ty.shape[0])):
            ...
            ax3.annotate(str('{:.2f}'.format(
                ty[index][-1])), xy=(tx[-1], offset), color=p3[index].get_facecolor().ravel())
            base += percent[-1]
...
def plot_reward(folder, game, name, num_steps, bin_size=10, smooth=1, time=None, save_filename='results.png', ipynb=False):
    ...
    if time is not None:
        plt.title(game + ' || Last 10: ' +
                  str(np.round(np.mean(ty[-10]))) + ' || Elapsed Time: ' + str(time))
    else:
